<div class="modal fade" id="schedule-training-{{ $schedule->vacancy->id }}">
    <div class="modal-dialog modal-lg">
        <form action="/backoffice/vacancy/{{ $schedule->vacancy->id }}/schedule-training" method="POST">
        @csrf
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Jadwal training {{ $schedule->vacancy->name }}</h4>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="card card-outline card-primary">
                                ...
                                <div class="card-body">
                                    <h5><b>Tanggal: </b> 
                                        @if ($schedule->vacancy->trainingSchedule)
                                            {{ \Carbon\Carbon::parse($schedule->vacancy->trainingSchedule->date)->locale('id')->isoFormat('dddd, D MMMM YYYY') }}
                                        @else
                                            Belum ditentukan
                                        @endif
                                    </h5>
                                    ...
                                    <h5><b>Waktu: </b>
                                        @if ($schedule->vacancy->trainingSchedule)
                                            {{ \Carbon\Carbon::parse($schedule->vacancy->trainingSchedule->time)->locale('id')->isoFormat('HH:mm') }}
                                        @else
                                            Belum ditentukan
                                        @endif
                                    </h5>
                                    ...
                                    <h5><b>Lokasi: </b> 
                                        @if ($schedule->vacancy->trainingSchedule)
                                            {{ $schedule->vacancy->trainingSchedule->location }}
                                        @else
                                            Belum ditentukan
                                        @endif
                                    </h5>
